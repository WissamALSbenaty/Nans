...
class ApiHelper {
  ...
  ApiHelper(this.errorsFactory,this.localizationManager) {
    ...
    _dio.interceptors.addAll([ LogInterceptor(
      requestBody: true, request: false, responseBody: true, requestHeader: true, responseHeader: false,)
    ]);
  }
  ...
  Future<ResponseModel> _sendRequest(RequestType requestType,String url,Map<String,dynamic> parameters,{bool isFormData=false})async{
    try {
      var response = await _dio.request('${Constants.baseUrl}/$url' ,
        ...
        data:requestType!=RequestType.GET? (isFormData? FormData.fromMap(parameters) : parameters) :null,
  ...
  Future<ResponseModel> post({required String url, required Map<String, dynamic> formData,bool isFormData=false}) async {
    return await _sendRequest(RequestType.POST, url,  formData,isFormData: isFormData);
  }
  Future<ResponseModel> patch({required String url, required Map<String, dynamic> formData}) async {
    return await _sendRequest(RequestType.PATCH, url,  formData);
  }
